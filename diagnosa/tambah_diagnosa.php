...
$ben = new diagnosa($pdo);
if(!empty($_POST['nama_diagnosa'])){
	$nama_diagnosa 			= $_POST['nama_diagnosa'];
	$created_by 		= $_SESSION['s_user'];
	if($ben->create($nama_diagnosa,$created_by)){
		$sg   = "ok";
...
<form id="forms" method="post" onSubmit="return submitForm('<?php echo $_SERVER['PHP_SELF']; ?>')" class="form-horizontal">
	<fieldset>
		<legend>Tambah Data Penyakit</legend>
		<span>
		 <?php
		...
		<div class="row-fluid">
			<div class="span6">
				...
				<label class="control-label" for="nama_diagnosa" >Penyakit</label>
				<div class="controls">
				<input type="text" id="nama_diagnosa" name="nama_diagnosa" required>
				</div>
				</div>
			</div>						
		</div>
